...
- `apps/CodexOpsConsole/`: a native macOS SwiftUI utility for viewing and
  ...
  backups, leases, and retained action results.
- `scripts/validate.py`: repo-level validation for all skills, the shared
  harness, vendored fallback copies, standalone skill-copy execution, and, in
  full mode, the Codex Ops Console native build and evidence gates.
...
Codex Ops Console can also aggregate explicitly configured separate homes and
...
The full gate also builds and tests Codex Ops Console, compiles its native
