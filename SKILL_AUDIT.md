...
- mandatory same-chat scheduled monitoring with executor/host binding, retained
  cursor, idempotent run keys, overlap refusal, first-run readiness, failure
  blocking, and terminal stop enforcement;
...
- realistic end-to-end self-tests for partial plumbing, implementation before
  ...
  history, scheduled-monitor readiness and overlap, and standalone copied
  execution.

What can improve: SQLite is shared by trusted accounts on one local repository
but does not by itself provide multi-host coordination. Scheduled monitoring is
truthfully unavailable on hosts without an automation-management tool; the
skill blocks instead of simulating persistence. Gantt rendering remains
capability gated. Product prioritization, task weights, estimates, and scope
...
- completion-ledger database-import export reruns the verifier while holding its exact
...
- every verified artifact-backed audit produces a fully dispositioned external
  ...
  then authorize one digest-bound transactional database import that preserves
  unrelated issues and permanent events and rejects raw, omitted, stale,
  changed, or concurrent input;
...
| Completion ledger | one shared software-owned database with permanent issue/event history; Markdown ledgers are forbidden | Markdown-presence and policy-fallback must-catch fixtures; database schema, integrity, transactional import, idempotency, transition, query, permanence, cross-account access, and release-gate tests |
