...
Codex Ops Console is a native macOS SwiftUI board for truthful local inventory
...
`Package.resolved`, and production `Sources/**/*.swift` paths and hashes:
...
```text
.build/app/CodexOpsConsole.app
```

A successful normal build writes a local build sidecar under
`.build/holyskills-packaging/<configuration>.json`. `--skip-build` never asks
