...
struct OpsConsoleView: View {
    ...
    var body: some View {
        ...
        .task(id: store.refreshIntervalSeconds) {
            await store.loadInventory()
            guard let interval = store.refreshIntervalSeconds else { return }
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 10) {
            Text(group.name)
                .font(.system(size: 15, weight: .bold))
                .lineLimit(2)
            DetailLine(label: "Runtime", value: group.projectPath.map(shortProject) ?? "No project")
