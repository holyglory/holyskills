...
def include_glob_explicitly_targets_dir(rel_dir: str, include_globs: list[str]) -> bool:
    ...
    for pattern in include_globs:
        ...
        if not literal_prefix:
            continue
        prefix_parts = PurePosixPath(literal_prefix.rstrip("/")).parts
        if not prefix_parts:
            continue
    ...
    for entry in entries:
        ...
        if line_units:
            for start_line, end_line, size_bytes in line_units:
                units.append(
                    AuditUnit(
                        unit_id=f"{entry.rel_path}#L{start_line}-{end_line}",
                        rel_path=entry.rel_path,
...
1. Find any explicit user journey, onboarding, workflow, route map, or product-flow description in the repo.
2. If journey documentation is missing or incomplete, draft the most reasonable frequent journey(s) from app intent, routes, visible copy, and code. Mark these as `draft-needs-user-confirmation`.
3. Walk the UI source along each journey. Estimate relevance for every visible navigation element or decision element you mention: `critical-always`, `primary-frequent`, `secondary-occasional`, or `rare-under-5-percent`.
4. Check whether critical and primary navigation elements are visible and easy to access compared with less relevant elements.
5. Check whether each journey step gives users enough information on mobile and desktop to make weighted decisions. Always-needed information should be visible; rare information should stay available within one click/tap; threshold warnings/notices should be visible without expansion.
6. Check whether the interface exposes a test mode or lightweight fixture path for visually exercising the journey without heavy load or production side effects.
7. Report concrete gaps with file evidence and visible copy when possible.
...
## Proposed Journeys
List confirmed journeys and any `draft-needs-user-confirmation` journeys.

## UI Source Journey Checks
Use a table with columns: Journey, Step, Files, Primary navigation/decision elements, Relevance estimate, Required information, Mobile/Desktop availability, Test mode evidence.
...
3. For each high-frequency journey, verify or plan a visual walk through desktop and narrow mobile viewports.
4. Check that navigation and primary decision elements are visible, relevant, and accessible.
5. Check that enough decision-making information is visible, important information is not cropped, rare information is one click/tap away, and threshold warnings are visible without drilling in.
6. Check theme consistency, readable font sizes, contrast, text/background colors, cropped content, accidental horizontal scroll on narrow screens, and whether heavy-load actions can be avoided in test mode.
7. When visual checks are applicable, cite the command/tool you used and at least one screenshot, trace, recording, or other artifact path/evidence in `Visual Tooling` or the `Evidence` column. If no repo-owned rendered UI exists, explicitly mark checks `not applicable` with evidence.
...
Use a table with columns: Journey, Viewport, Route/screen, Evidence, Navigation visibility, Decision information, Visual quality, Result.
...
def write_outputs(
    ...
    verifier_args = [
        sys.executable,
        str(Path(__file__).resolve().with_name("verify_audit_results.py")),
        "--manifest",
...
def render_index(repo: Path, out_dir: Path, manifest: dict) -> str:
    rows = "\n".join(
        f"| {batch['id']} | `{batch['prompt']}` | {batch['file_count']} | {batch.get('coverage_unit_count', batch['file_count'])} | {batch['interface_file_count']} | {batch['byte_count']} | {batch['purpose']} |"
        for batch in manifest["batches"]
    )
    if not rows:
